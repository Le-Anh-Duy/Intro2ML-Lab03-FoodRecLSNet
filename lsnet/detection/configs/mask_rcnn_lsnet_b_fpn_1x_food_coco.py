model = dict(
    type='MaskRCNN',
    pretrained=None,
    backbone=dict(type='lsnet_b', pretrained=None, frozen_stages=-1),
    neck=dict(
        type='LSNetFPN',
...
data = dict(
    ...
    train=dict(
        type='CocoDataset',
        ann_file='dataset/UECFOODPIXCOMPLETE_COCO/train/annotation.json',
        img_prefix='dataset/UECFOODPIXCOMPLETE_COCO/',
        pipeline=[
            dict(type='LoadImageFromFile'),
            dict(type='LoadAnnotations', with_bbox=True, with_mask=True),
            dict(type='Resize', img_scale=(1333, 800), keep_ratio=True),
    ...
    val=dict(
        type='CocoDataset',
        ann_file='dataset/UECFOODPIXCOMPLETE_COCO/test/annotation.json',
        img_prefix='dataset/UECFOODPIXCOMPLETE_COCO/',
        pipeline=[
            dict(type='LoadImageFromFile'),
            dict(
...
log_level = 'INFO'
load_from = './pretrain/lsnet_b_maskrcnn.pth'
resume_from = None
workflow = [('train', 1)]
total_epochs = 12
classes = (
    'rice', 'eels on rice', 'pilaf', "chicken-'n'-egg on rice",
    'pork cutlet on rice', 'beef curry', 'sushi', 'chicken rice', 'fried rice',
    'tempura bowl', 'bibimbap', 'toast', 'croissant', 'roll bread',
    'raisin bread', 'chip butty', 'hamburger', 'pizza', 'sandwiches',
    'udon noodle', 'tempura udon', 'soba noodle', 'ramen noodle',
    'beef noodle', 'tensin noodle', 'fried noodle', 'spaghetti',
    'Japanese-style pancake', 'takoyaki', 'gratin', 'sauteed vegetables',
    'croquette', 'grilled eggplant', 'sauteed spinach', 'vegetable tempura',
    'miso soup', 'potage', 'sausage', 'oden', 'omelet', 'ganmodoki', 'jiaozi',
    'stew', 'teriyaki grilled fish', 'fried fish', 'grilled salmon',
    'salmon meuniere', 'sashimi', 'grilled pacific saury', 'sukiyaki',
    'sweet and sour pork', 'lightly roasted fish', 'steamed egg hotchpotch',
    ...
    'cabbage roll', 'rolled omelet', 'egg sunny-side up', 'fermented soybeans',
    'cold tofu', 'egg roll', 'chilled noodle', 'stir-fried beef and peppers',
    'simmered pork', 'boiled chicken and vegetables', 'sashimi bowl',
    'sushi bowl', 'fish-shaped pancake with bean jam',
    'shrimp with chill source', 'roast chicken', 'steamed meat dumpling',
    ...
    'dipping noodles', 'hot dog', 'french fries', 'mixed rice',
    'goya chanpuru', 'others', 'beverage')
work_dir = './finetuned/3'
auto_resume = False
gpu_ids = range(0, 8)
